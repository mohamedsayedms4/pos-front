...
const SalesAnalytics = () => {
  ...
  // Loaders
  const loadSummary = useCallback(async (branchId = selectedBranchId) => {
    setSummaryLoading(true);
    try {
      const res = await Api.getSalesSummary('', branchId);
      setSummary(res);
    } catch (err) { toast(err.message, 'error'); }
    finally { setSummaryLoading(false); }
  }, [toast, selectedBranchId]);

  const loadHourly = useCallback(async (date, branchId = selectedBranchId) => {
    setHourlyLoading(true);
    try {
      const res = await Api.getHourlyAnalytics(date, branchId);
      setHourlyData(res);
    } catch (err) { toast(err.message, 'error'); }
    finally { setHourlyLoading(false); }
  }, [toast, selectedBranchId]);

  const loadCashiers = useCallback(async (date, branchId = selectedBranchId) => {
    setCashierLoading(true);
    try {
      const res = await Api.getCashierAnalytics(date, branchId);
      setCashierData(res);
    } catch (err) { toast(err.message, 'error'); }
    finally { setCashierLoading(false); }
  }, [toast, selectedBranchId]);

  const loadProducts = useCallback(async (date, branchId = selectedBranchId) => {
    setProductLoading(true);
    try {
      const res = await Api.getProductAnalytics(date, branchId);
      setProductData(res);
    } catch (err) { toast(err.message, 'error'); }
    finally { setProductLoading(false); }
  }, [toast, selectedBranchId]);

  const loadReturns = useCallback(async (date, branchId = selectedBranchId) => {
    setReturnLoading(true);
    try {
      const res = await Api.getReturnAnalytics(date, branchId);
      setReturnData(res);
    } catch (err) { toast(err.message, 'error'); }
    finally { setReturnLoading(false); }
  }, [toast, selectedBranchId]);

  // Sync context branches & default selected branch
  useEffect(() => {
  ...
  // Effects
  useEffect(() => { loadSummary(selectedBranchId); }, [loadSummary, selectedBranchId]);
  useEffect(() => { loadHourly(hourlyDate, selectedBranchId); }, [hourlyDate, loadHourly, selectedBranchId]);
  useEffect(() => { loadCashiers(cashierDate, selectedBranchId); }, [cashierDate, loadCashiers, selectedBranchId]);
  useEffect(() => { loadProducts(productDate, selectedBranchId); }, [productDate, loadProducts, selectedBranchId]);
  useEffect(() => { loadReturns(returnDate, selectedBranchId); }, [returnDate, loadReturns, selectedBranchId]);
      ...
      <div className="card-header" style={{ marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px' }}>
        ...
        {Api.isAdminOrBranchManager() && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            >
              <option value="" style={{ background: '#151515', color: '#ffffff' }}>جميع الفروع</option>
              {branches.map(b => (
                <option key={b.id} value={b.id} style={{ background: '#151515', color: '#ffffff' }}>{b.name}</option>
              ))}
            </select>
          </div>
        )}
