...
const SuperAdminLeadCommunications = () => {
  ...
  const [scheduleAfterSubmit, setScheduleAfterSubmit] = useState(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (scheduleAfterSubmit) {
        ...
        setScheduleAfterSubmit(false); // reset
      }
    } catch (err) {
    ...
    <div className="page-section">
      <div className="card" style={{ marginBottom: '20px' }}>
        <div className="card-header" style={{ display: 'flex', alignItems: 'center', gap: '15px', padding: '15px 20px' }}>
          ...
          <h3 style={{ margin: 0 }}><i className="fa-solid fa-phone"></i> سجل تواصل العميل المحتمل: {leadName}</h3>
        </div>
      </div>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-start', gap: '10px' }}>
                <button 
                  ...
                  onClick={() => setScheduleAfterSubmit(false)}
                >
